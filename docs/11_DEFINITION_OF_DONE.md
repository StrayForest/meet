...
- **Product/business:** for user-visible/growth/B2B work, name the PMF/liquidity/GTM/organizer hypothesis affected and the measurable downstream outcome. Do not treat installs, registrations, page views or time-in-app as success when the goal is IRL participation. Preserve `fi/en/ru` launch support where the flow is user-visible. The validation/business contract gate is mandatory for scope-changing user-visible work.

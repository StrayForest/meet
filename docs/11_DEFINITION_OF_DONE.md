...
- **Docs:** update only affected source-of-truth docs/plan/generated artifacts; do not duplicate specs. `node scripts/check-context-budget.mjs`, `node scripts/check-architecture-contracts.mjs`, `node scripts/check-business-contracts.mjs` and `node scripts/check-validation-scope.mjs` pass.

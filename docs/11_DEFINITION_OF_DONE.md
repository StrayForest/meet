...
- **Scope discipline:** new architecture/features require a concrete product, safety, legal, reliability or measured-scale reason. Prefer the smallest change that advances the minimum proof loop; speculative completeness is not Done.
...
- **Docs:** update only affected source-of-truth docs/plan/generated artifacts; do not duplicate specs. `node scripts/check-context-budget.mjs`, `node scripts/check-architecture-contracts.mjs` and `node scripts/check-business-contracts.mjs` pass.
